        ...
        <body style="height: 100vh; display: flex; justify-content: center; align-items: center; background-color: #f8f9fa;">
          <div class="container">
              <div class="card" style="max-width: 600px; margin: auto;">
                  <div class="card-body">
                      <h2 class="text-center mb-4">Edit Jenis Kategori</h2>
                      ...
                      <form method="POST" action="{{ route('admin.jenis.update', $jenis->id) }}"> 
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="editName_jenis" class="form-label">Nama Jenis Barang</label>
                            <input type="text" class="form-control" id="editName_jenis" name="name_jenis" value="{{ $jenis->name_jenis }}" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" aria-label="Default select example" id="editKategori" name="kategori" required>
                                ...
                                @else
                                    ...
                                    <option value="{{ $kategori->nama_kategori }}" {{ $kategori->nama_kategori == $jenis->kategori ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
