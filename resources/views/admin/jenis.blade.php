...
<body style="height: 100vh; overflow: hidden;">
    ...
    <div class="content" style="margin-left: 250px; padding: 20px; overflow-y: auto;">
      ...
      <div class="d-flex justify-content-between align-items-center mt-3">
        ...
        <a href="{{ route('admin.jenis.create') }}" class="btn btn-primary bi bi-plus-circle"> Tambah</a>
    
        <!-- Form Search di sebelah kanan -->
        <div class="search-bar ml-auto">
            <form action="/jenis" class="input-group" style="width: 300px;">
      ...
      <table class="table table-hover">
          <thead>
            ...
            <tbody class="table-group-divider">
                ...
                <tr>
                    <th scope="row">{{ ($jenis->currentPage() - 1) * $jenis->perPage() + $loop->iteration }}</th>
                    <td>{{ $item->name_jenis }}</td>
                    <td>{{ $item->kategori }}</td>
                    <td>
                          <a href="{{ route('admin.jenis.edit', $item->id) }}" class="btn btn-warning btn-sm"><i class="bi bi-pencil-square"></i> Edit</a>
                          <form action="{{ route('admin.jenis.destroy', $item->id) }}" method="POST" class="d-inline">
                              @csrf
                              @method('DELETE')
                              <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus Barang Jenis ini?');"><i class="bi bi-trash"></i> Hapus</button>
                          </form>
                      </td>
                  </tr>
              @endforeach
      ...
      <div class="d-flex justify-content-between align-items-center mt-3">
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination mt-2">
                ...
                @else
                    <li class="page-item"><a class="page-link" href="{{ $jenis->previousPageUrl() }}">Previous</a></li>
                @endif
    
                {{-- Pagination Elements --}}
                @foreach ($jenis->links()->elements as $element)
                    {{-- Array Of Links --}}
                    @if (is_array($element))
                ...
                @if ($jenis->hasMorePages())
                    <li class="page-item"><a class="page-link" href="{{ $jenis->nextPageUrl() }}">Next</a></li>
                @else
